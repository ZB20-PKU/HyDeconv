...
```
**Important:** Please adjust the versions of `cudatoolkit`, `cupy-cuda`, `torch`, `torchvision`, and `torchaudio` to match your specific GPU if necessary. Additionally, if any package reports an error during `pip install` such as "Microsoft Visual C++ 14.0 or greater is required", you will need to install the **Microsoft C++ Build Tools**:
