...
import numpy as np
import tifffile
from src.default_config import default_config_Hybrid_WFM
from src_BF.BF_WFM import BF_WFM
from src_Hessian.core.Hessian_denoise import Hessian_denoise_WFM
from src_TDV.TDV_denoise import TDV_denoise_WFM
from src_Sparse.iterative_deconv.Sparse_deconvolution import Sparse_deconvolution_WFM
import matplotlib
...
# 修改后的Hybrid_WFM类，支持进度回调
class Hybrid_WFM_:    
    def recon(self, args, progress_callback=None):
        # 创建四个模块，并传入进度回调
        BF = BF_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
        args.bf_result = BF.recon()
        
        if args.Hessian_iteration_number > 0:
            Hessian = Hessian_denoise_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
            args.hessian_result = Hessian.recon()
        else:
            args.hessian_result = args.bf_result
        
        if args.TDV_iteration_number > 0:
            TDV = TDV_denoise_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
            args.tdv_result = TDV.recon()
        else:
            args.tdv_result = args.hessian_result

        if args.Sparse_iteration_number > 0:
            Sparse = Sparse_deconvolution_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
...
class Hybrid_WFM:
    ...
    def recon(self, args, progress_callback=None):
        ...
        # 创建四个模块，并传入进度回调
        self.bf_sim = BF_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
        if self._stop_requested:
            return {}
        ...
        if args.Hessian_iteration_number > 0 and not self._stop_requested:
            self.hessian = Hessian_denoise_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
            if self._stop_requested:
        ...
        if args.TDV_iteration_number > 0 and not self._stop_requested:
            self.tdv = TDV_denoise_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
            if self._stop_requested:
                return {
                    'bf_result': args.bf_result,
                    'bf_hessian_result': args.hessian_result
        ...
        if args.Sparse_iteration_number > 0 and not self._stop_requested:
            self.sparse = Sparse_deconvolution_WFM(args, lambda p: progress_callback(p) if progress_callback else None)
            if self._stop_requested:
                return {
                    'bf_result': args.bf_result,
                    'bf_hessian_result': args.hessian_result,
...
class ReconstructionThread(QThread):
    ...
    def __init__(self, args):
        ...
        self.hybrid_wfm = None  # 存储Hybrid_SIM实例
        
    def run_(self):
        try:
            # 创建Hybrid_WFM实例
            hybrid_wfm = Hybrid_WFM()
            
            # 执行重建，传入进度回调函数
            results = hybrid_wfm.recon(self.args, self.update_progress)
            if self._is_running:
    ...
    def run(self):
        try:
            # 创建Hybrid_SIM实例
            self.hybrid_wfm = Hybrid_WFM()
            
            # 执行重建，传入进度回调函数
            results = self.hybrid_wfm.recon(self.args, self.update_progress)
...
class ParameterWidget(QWidget):
    ...
    def __init__(self, param_name, param_type, default_value, tooltip="", unit="", parent=None):
        ...
        label_text = label_text.replace('Wfm', 'WFM')
        label_text = label_text.replace('Fm', 'WFM')
        ...
        if param_type == "file":
            ...
            if param_name in ['WFM_excitation_wavelength', 'FM_emission_wavelength', 'FM_Raw_pixel_size']:
                # 默认值是以米为单位的，转换为nm显示
    ...
    def browse_file(self):
        ...
        # 根据参数名称设置默认文件夹
        if self.param_name == 'WFM_BF_PSF_path':
    ...
    def browse_file_(self):
        ...
        # 根据参数名称设置默认文件夹
        if self.param_name == 'WFM_BF_PSF_path':
            default_dir = os.path.join(root_dir, 'src_Optics/WFM_BF_PSF')
            file_filter = "TIFF Files (*.tif *.tiff);;All Files (*)"
        elif self.param_name == 'WFM_Recon_OTF_path':
            default_dir = os.path.join(root_dir, 'src_Optics/WFM_Recon_OTF')
            file_filter = "TIFF Files (*.tif *.tiff);;All Files (*)"
        elif self.param_name == 'TDV_model_path':
            default_dir = os.path.join(root_dir, 'src_model')
            file_filter = "Model Files (*.pth);;All Files (*)"
        else:
            # 对于其他文件，使用当前设置的路径的目录或根目录
            current_path = self.widget.text()
    ...
    def reset_value(self):
        if self.param_type == "file" or self.param_type == "str":
            ...
            # 特殊处理：对于波长和像素尺寸，显示为nm单位的整数值
            if self.param_name in ['WFM_excitation_wavelength', 'FM_emission_wavelength', 'FM_Raw_pixel_size']:
                # 默认值是以米为单位的，转换为nm显示
...
class ModuleWidget(QGroupBox):
    ...
    def __init__(self, title, parameters, parent=None):
        ...
        title = title.replace('Wfm', 'WFM')
        title = title.replace('Fm', 'WFM')
...
class WFMReconstructionGUI(QMainWindow):
    # 进度更新信号
    progress_update_signal = pyqtSignal(int)
    
    def __init__(self):
        super().__init__()
        self.args = default_config_Hybrid_WFM()  # 使用模拟的Args类
        self.results = {}
    ...
    def init_ui(self):
        self.setWindowTitle("Hybrid-WFM Reconstruction")
        # 修改：设置窗口大小为1200x900
        ...
        # 标题
        title_label = QLabel("Hybrid-WFM Reconstruction Parameters")
        ...
        wfm_params = {
            'WFM_raw_data_path': {
                'type': 'file',
                'default': './Demo_Recon_WFM.tif',
                'tooltip': 'Path to WFM raw data TIFF file'
            },
            'FM_Raw_pixel_size': {
                'type': 'float',
                'default': 65e-9,
            ...
            'FM_emission_NA': {
                'type': 'float',
                'default': 1.4,
                'tooltip': 'Emission NA parameter',
                'unit': ''
            }
        }
        self.wfm_widget = ModuleWidget("BF-WFM Parameters", wfm_params)
        self.tab_widget.addTab(self.wfm_widget, "BF-WFM")
        ...
        tdv_params = {
            'TDV_model_path': {
                'type': 'file',
                'default': './src_Hybrid/src_model/TDV_WFM_Peroxisome.pth',
        ...
        # Sparse Deconvolution 模块 - 添加FM_emission_wavelength和FM_Raw_pixel_size参数
        sparse_params = {
            'Sparse_NA': {
                'type': 'float',
                'default': 1,
        ...
        # 创建四个图像显示容器
        self.bf_display = ImageDisplayContainer("BF-WFM Reconstruction")
        self.bf_hessian_display = ImageDisplayContainer("BF+Hessian-WFM Reconstruction")
        self.bf_hessian_tdv_display = ImageDisplayContainer("BF+Hessian+TDV-WFM Reconstruction")
        self.hybrid_display = ImageDisplayContainer("Hybrid-WFM Reconstruction")
    ...
    def update_progress(self, value):
        ...
        if value == 0:
            ...
            self.status_label.setText(f"Executing BF-WFM reconstruction ...")
        elif value < 50:
    ...
    def disable_controls(self):
        ...
        # 禁用参数标签页中的所有控件
        self.wfm_widget.set_enabled(False)
    ...
    def enable_controls(self):
        ...
        # 启用参数标签页中的所有控件
        self.wfm_widget.set_enabled(True)
    ...
    def load_default_parameters(self):
        ...
        # 更新所有参数控件的值为默认值
        for widget in [self.wfm_widget, self.hessian_widget, self.tdv_widget, self.sparse_widget]:
    ...
    def load_previous_parameters(self):
        ...
        try:
            # 构建默认文件名
            try:
                filedir, filename = os.path.split(self.args.WFM_raw_data_path)
                base_name = filedir +'/'
    ...
    def export_current_parameters(self):
        ...
        try:
            ...
            try:
                ...
                # 如果 fname 不存在，使用空字符串作为基础名称
                base_name = "Demo_Recon_WFM_Recon_Parameters.json"
    ...
    def export_images(self):
        ...
        # 创建复选框
        self.export_bf_checkbox = QCheckBox("BF-WFM Reconstruction")
        self.export_bf_hessian_checkbox = QCheckBox("BF+Hessian-WFM Reconstruction")
        self.export_bf_hessian_tdv_checkbox = QCheckBox("BF+Hessian+TDV-WFM Reconstruction")
        self.export_hybrid_checkbox = QCheckBox("Hybrid-WFM Reconstruction")
    ...
    def collect_all_parameters(self):
        ...
        # WFM参数
        params['wfm'] = self.wfm_widget.get_values()
    ...
    def set_parameters_from_dict(self, params):
        ...
        try:
            ...
            # 设置WFM参数
            if 'wfm' in params:
                for key, value in params['wfm'].items():
                    if key in self.wfm_widget.parameters:
                        self.wfm_widget.parameters[key].set_value(value)
    ...
    def collect_parameters(self):
        ...
        # 收集WFM参数
        wfm_values = self.wfm_widget.get_values()
        for key, value in wfm_values.items():
