...
import numpy as np
import tifffile
from src.default_config import default_config_Hybrid_FM
from src_Hessian.core.Hessian_denoise import Hessian_denoise_FM
from src_TDV.TDV_denoise import TDV_denoise_FM
from src_Sparse.iterative_deconv.Sparse_deconvolution import Sparse_deconvolution_FM
import matplotlib
...
class Hybrid_FM_:    
    def recon(self, args, progress_callback=None):
        ...
        if args.Hessian_iteration_number > 0:
            Hessian = Hessian_denoise_FM(args, lambda p: progress_callback(p) if progress_callback else None)
            args.hessian_result = Hessian.recon()
        else:
            # 如果Hessian迭代数为0，直接使用原始数据
            ImageLib.getInfo(args.Raw_data_path)
            args.hessian_result = ImageLib.read(args.Raw_data_path, to_tensor=False)
        
        if args.TDV_iteration_number > 0:
            TDV = TDV_denoise_FM(args, lambda p: progress_callback(p) if progress_callback else None)
            args.tdv_result = TDV.recon()
        else:
            args.tdv_result = args.hessian_result

        if args.Sparse_iteration_number > 0:
            Sparse = Sparse_deconvolution_FM(args, lambda p: progress_callback(p) if progress_callback else None)
            args.sparse_background, args.sparse_result = Sparse.recon()
        else:
            args.sparse_result = args.tdv_result
            # 如果Sparse迭代数为0，设置背景为0
            args.sparse_background = np.zeros_like(args.tdv_result)

        # 返回所有结果
        return {
            'hessian_result': args.hessian_result,
            'hessian_tdv_result': args.tdv_result,
            'hybrid_background': args.sparse_background,
...
class Hybrid_FM:
    ...
    def recon(self, args, progress_callback=None):
        ...
        # 创建模块，并传入进度回调
        if args.Hessian_iteration_number > 0:
            self.hessian = Hessian_denoise_FM(args, lambda p: progress_callback(p) if progress_callback else None)
            if self._stop_requested:
                return {}
        ...
        if args.TDV_iteration_number > 0 and not self._stop_requested:
            self.tdv = TDV_denoise_FM(args, lambda p: progress_callback(p) if progress_callback else None)
            if self._stop_requested:
                return {
                    'hessian_result': args.hessian_result
                }
        ...
        if args.Sparse_iteration_number > 0 and not self._stop_requested:
            self.sparse = Sparse_deconvolution_FM(args, lambda p: progress_callback(p) if progress_callback else None)
            if self._stop_requested:
                return {
                    'hessian_result': args.hessian_result,
                    'hessian_tdv_result': args.tdv_result
            ...
            args.sparse_background, args.sparse_result = self.sparse.recon()
        else:
            args.sparse_result = args.tdv_result
            # 如果Sparse迭代数为0，设置背景为0
            args.sparse_background = np.zeros_like(args.tdv_result)
...
class ReconstructionThread_(QThread):
    """重建线程, 避免阻塞GUI"""
    ...
    def run(self):
        try:
            ...
            # 执行重建，传入进度回调函数
            results = hybrid_fm.recon(self.args, self.update_progress)
            if self._is_running:
...
class ReconstructionThread(QThread):
    ...
    def __init__(self, args):
        ...
        self.hybrid_fm = None  # 存储Hybrid_FM实例
        
    def run(self):
        try:
            # 创建Hybrid_FM实例
            self.hybrid_fm = Hybrid_FM()
            
            # 执行重建，传入进度回调函数
            results = self.hybrid_fm.recon(self.args, self.update_progress)
...
class ParameterWidget(QWidget):
    ...
    def __init__(self, param_name, param_type, default_value, tooltip="", unit="", parent=None):
        ...
        # label_text = label_text.replace(Microscopy_Type_small, Microscopy_Type)
        ...
        if param_type == "file":
            ...
            if param_name in ['STED_excitation_wavelength', 'STED_emission_wavelength', 'STED_Raw_pixel_size', 'Emission_wavelength', 'Raw_pixel_size']:
                # 默认值是以米为单位的，转换为nm显示
    ...
    def browse_file(self):
        ...
        # 根据参数名称设置默认文件夹
        if self.param_name == 'TDV_model_path':
    ...
    def reset_value(self):
        if self.param_type == "file" or self.param_type == "str":
            ...
            # 特殊处理：对于波长和像素尺寸，显示为nm单位的整数值
            if self.param_name in ['STED_excitation_wavelength', 'STED_emission_wavelength', 'STED_Raw_pixel_size', 'Emission_wavelength', 'Raw_pixel_size']:
                # 默认值是以米为单位的，转换为nm显示
...
class ModuleWidget(QGroupBox):
    ...
    def __init__(self, title, parameters, parent=None):
        ...
        # title = title.replace(Microscopy_Type_small, Microscopy_Type)
...
class STEDReconstructionGUI(QMainWindow):
    # 进度更新信号
    progress_update_signal = pyqtSignal(int)
    
    def __init__(self):
        super().__init__()
        self.args = default_config_Hybrid_FM()  # 使用模拟的Args类
        self.results = {}
        ...
        self._reconstruction_started = False  # 添加这个标志
    ...
    def handle_stop_completed(self):
        """处理停止完成"""
        self._reconstruction_started = False  # 重置标志
        # 恢复按钮状态
        self.stop_recon_btn.setEnabled(False)
    ...
    def init_ui(self):
        self.setWindowTitle("Hybrid-"+Microscopy_Type+" Reconstruction")
        # 修改：设置窗口大小为1200x900
        ...
        # 标题
        title_label = QLabel("Hybrid-MPM Reconstruction Parameters")
        ...
        # Hessian Denoise 模块 - 添加Raw_data_path参数作为第一个参数
        hessian_params = {
            'Raw_data_path': {
                'type': 'file',
                'default': './Demo_Recon_'+Microscopy_Type+'.tif',
                'tooltip': 'Path to '+Microscopy_Type+' raw data TIFF file'
            },
            'Hessian_fidelity': {
                'type': 'float',
                'default': 50,
        ...
        tdv_params = {
            'TDV_model_path': {
                'type': 'file',
                'default': './src_Hybrid/src_model/TDV_'+Microscopy_Type+'_ATP.pth',
        ...
        # Sparse Deconvolution 模块 - 添加新参数
        sparse_params = {
            'Rolling_ball_radius': {
                'type': 'float',
                'default': 10,
                'tooltip': 'Rolling ball radius for background estimation',
                'unit': 'pixels'
            },
            'Emission_wavelength': {
                'type': 'float',
        ...
        # 创建四个图像显示容器 - 修改为STED相关
        self.hessian_display = ImageDisplayContainer("Hessian-"+Microscopy_Type+" Reconstruction")
        self.hessian_tdv_display = ImageDisplayContainer("Hessian+TDV-"+Microscopy_Type+" Reconstruction")
        self.hybrid_background_display = ImageDisplayContainer("Hybrid-"+Microscopy_Type+" Background")
        self.hybrid_display = ImageDisplayContainer("Hybrid-"+Microscopy_Type+" Reconstruction")
        
        # 添加到网格
        image_grid.addWidget(self.hessian_display, 0, 0, Qt.AlignCenter)
        image_grid.addWidget(self.hessian_tdv_display, 0, 1, Qt.AlignCenter)
        image_grid.addWidget(self.hybrid_background_display, 1, 0, Qt.AlignCenter)
        ...
        # 存储所有图像显示控件，用于联动
        self.image_displays = [
            self.hessian_display.image_widget,
            self.hessian_tdv_display.image_widget,
            self.hybrid_background_display.image_widget,
            self.hybrid_display.image_widget
        ]
        
        # 存储所有图像显示容器，用于禁用交互
        self.image_containers = [
            self.hessian_display,
            self.hessian_tdv_display,
            self.hybrid_background_display,
            self.hybrid_display
        ]
    ...
    def update_progress(self, value):
        ...
        if value == 0:
            # 只有在重建未开始时才显示"Ready !"
            if not self._reconstruction_started:
                self.status_label.setText("Ready !")
            # 否则保持当前状态（可能是"Starting reconstruction ..."）
        elif value < 33:
            self.status_label.setText(f"Executing Hessian Denoise ...")
        elif value < 66:
            self.status_label.setText(f"Executing TDV Denoise ...")
    ...
    def disable_controls(self):
        ...
        self.stop_recon_btn.setEnabled(True)  # 启用停止按钮
    ...
    def enable_controls(self):
        ...
        # 启用运行按钮，禁用停止按钮
        self.run_btn.setEnabled(True)
        self.stop_recon_btn.setEnabled(False)  # 禁用停止按钮
    ...
    def load_default_parameters(self):
        ...
        # 更新所有参数控件的值为默认值
        for widget in [self.hessian_widget, self.tdv_widget, self.sparse_widget]:
    ...
    def load_previous_parameters(self):
        ...
        try:
            # 构建默认文件名
            try:
                filedir, filename = os.path.split(self.args.Raw_data_path)
                base_name = filedir +'/'
    ...
    def export_current_parameters(self):
        ...
        try:
            ...
            try:
                ...
                # 如果 fname 不存在，使用空字符串作为基础名称
                base_name = "Demo_Recon_"+Microscopy_Type+"_Recon_Parameters.json"
    ...
    def export_images(self):
        ...
        dialog.setMinimumWidth(450)  # 增加宽度以容纳更多控件
        ...
        # 创建复选框 - 修改为STED相关选项
        self.export_hessian_checkbox = QCheckBox("Hessian-"+Microscopy_Type+" Reconstruction")
        self.export_hessian_tdv_checkbox = QCheckBox("Hessian+TDV-"+Microscopy_Type+" Reconstruction")
        self.export_hybrid_background_checkbox = QCheckBox("Hybrid-"+Microscopy_Type+" Background")
        self.export_hybrid_checkbox = QCheckBox("Hybrid-"+Microscopy_Type+" Reconstruction")
        
        # 默认只选择Hybrid-STED
        self.export_hybrid_checkbox.setChecked(True)
        
        full_data_layout.addWidget(self.export_hessian_checkbox)
        full_data_layout.addWidget(self.export_hessian_tdv_checkbox)
        full_data_layout.addWidget(self.export_hybrid_background_checkbox)
        ...
        # 堆栈类型选择
        stack_type_widget = QWidget()
        stack_type_layout = QHBoxLayout()
        stack_type_layout.setContentsMargins(20, 5, 5, 5)  # 缩进
        
        stack_type_label = QLabel("Stack type:")
        self.stack_type_combo = QComboBox() 
        self.stack_type_combo.addItem("Type 1: Hessian-Background, Hessian+TDV-Background, Hybrid")
        self.stack_type_combo.addItem("Type 2: Hessian, Hessian+TDV, Hybrid+Background")
        self.stack_type_combo.setEnabled(False)  # 初始不可用
        
        # 连接复选框状态改变信号
        self.export_frame_stack_checkbox.toggled.connect(self.stack_type_combo.setEnabled)
        
        stack_type_layout.addWidget(stack_type_label)
        stack_type_layout.addWidget(self.stack_type_combo)
        stack_type_layout.addStretch()
        
        stack_type_widget.setLayout(stack_type_layout)
        ...
        frame_stack_layout.addWidget(stack_type_widget)
        ...
        # 添加一些说明
        # note_label = QLabel("Note: Type 1 includes Hessian, Hessian+TDV, and Hybrid+Background. Type 2 includes Hessian-Background, Hessian+TDV-Background, and Hybrid.")
        ...
        # 获取导出选项
        export_options = {
            'hessian': self.export_hessian_checkbox.isChecked(),
            'hessian_tdv': self.export_hessian_tdv_checkbox.isChecked(),
            'hybrid_background': self.export_hybrid_background_checkbox.isChecked(),
            'hybrid': self.export_hybrid_checkbox.isChecked(),
            'frame_stack': self.export_frame_stack_checkbox.isChecked(),
            'stack_type': self.stack_type_combo.currentText(),
            'frame_index': self.frame_spinbox.value() if self.export_frame_stack_checkbox.isChecked() else None
        }
        
        # 检查是否有选中的数据
        has_full_data = any([export_options['hessian'], export_options['hessian_tdv'], 
                            export_options['hybrid_background'], export_options['hybrid']])
        ...
        try:
            ...
            # 计算总文件数（包括可能的堆栈文件）
            total_files = sum([1 for v in [export_options['hessian'], export_options['hessian_tdv'], 
                                        export_options['hybrid_background'], export_options['hybrid']] if v])
            ...
            if has_full_data:
                # 导出Hessian-STED数据
                if export_options['hessian'] and 'hessian_result' in self.results:
                    data = self.results['hessian_result']
                    filename = self.save_image_data(data, self.args.fname+'_Hessian', save_dir)
                    if filename:
                        success_count += 1
                    self.progress_update_signal.emit((success_count * 100) // total_files)
                    QApplication.processEvents()
                
                # 导出Hessian+TDV-STED数据
                if export_options['hessian_tdv'] and 'hessian_tdv_result' in self.results:
                    data = self.results['hessian_tdv_result']
                    filename = self.save_image_data(data, self.args.fname+'_Hessian+TDV', save_dir)
                    if filename:
                        success_count += 1
                    self.progress_update_signal.emit((success_count * 100) // total_files)
                    QApplication.processEvents()
                
                # 导出Hybrid-STED Background数据
                if export_options['hybrid_background'] and 'hybrid_background' in self.results:
                    data = self.results['hybrid_background']
                    filename = self.save_image_data(data, self.args.fname+'_Hybrid_Background', save_dir)
                    if filename:
                        success_count += 1
                    self.progress_update_signal.emit((success_count * 100) // total_files)
                    QApplication.processEvents()
            ...
            if export_options['frame_stack']:
                try:
                    ...
                    # 根据选择的堆栈类型构建不同的堆栈
                    stack_type = export_options['stack_type']
                    
                    # 交换 Type 1 和 Type 2 的内容
                    if "Type 1" in stack_type:
                        # 现在的 Type 1: Hessian-Background, Hessian+TDV-Background, Hybrid
                        stack_frames = []
                        
                        # 提取Background帧
                        background_frame = None
                        if 'hybrid_background' in self.results:
                            if len(self.results['hybrid_background'].shape) == 3:
                                if internal_frame_idx < self.results['hybrid_background'].shape[0]:
                                    background_frame = self.results['hybrid_background'][internal_frame_idx]
                                else:
                                    background_frame = self.results['hybrid_background'][-1]
                            else:
                                background_frame = self.results['hybrid_background']
                        
                        # 提取Hessian-Background帧
                        if 'hessian_result' in self.results and background_frame is not None:
                            if len(self.results['hessian_result'].shape) == 3:
                                if internal_frame_idx < self.results['hessian_result'].shape[0]:
                                    hessian_frame = self.results['hessian_result'][internal_frame_idx]
                                else:
                                    hessian_frame = self.results['hessian_result'][-1]
                            else:
                                hessian_frame = self.results['hessian_result']
                        ...
                        if 'hessian_tdv_result' in self.results and background_frame is not None:
                            if len(self.results['hessian_tdv_result'].shape) == 3:
                                if internal_frame_idx < self.results['hessian_tdv_result'].shape[0]:
                                    hessian_tdv_frame = self.results['hessian_tdv_result'][internal_frame_idx]
                                else:
                                    hessian_tdv_frame = self.results['hessian_tdv_result'][-1]
                            else:
                                hessian_tdv_frame = self.results['hessian_tdv_result']
                        ...
                        if 'hybrid_result' in self.results:
                            if len(self.results['hybrid_result'].shape) == 3:
                                if internal_frame_idx < self.results['hybrid_result'].shape[0]:
                                    hybrid_frame = self.results['hybrid_result'][internal_frame_idx]
                                else:
                                    hybrid_frame = self.results['hybrid_result'][-1]
                            else:
                                hybrid_frame = self.results['hybrid_result']
                            stack_frames.append(hybrid_frame)
                        
                        stack_name = f"{self.args.fname}_Frame{user_frame_idx}_Comparison_Type1"
                        
                    else:  # Type 2
                        # 现在的 Type 2: Hessian, Hessian+TDV, Hybrid+Background
                        stack_frames = []
                        
                        # 提取Hessian帧
                        if 'hessian_result' in self.results:
                            if len(self.results['hessian_result'].shape) == 3:
                                if internal_frame_idx < self.results['hessian_result'].shape[0]:
                                    frame_data = self.results['hessian_result'][internal_frame_idx]
                                else:
                                    frame_data = self.results['hessian_result'][-1]
                            else:
                                frame_data = self.results['hessian_result']
                            stack_frames.append(frame_data)
                        
                        # 提取Hessian+TDV帧
                        if 'hessian_tdv_result' in self.results:
                            if len(self.results['hessian_tdv_result'].shape) == 3:
                                if internal_frame_idx < self.results['hessian_tdv_result'].shape[0]:
                                    frame_data = self.results['hessian_tdv_result'][internal_frame_idx]
                                else:
                                    frame_data = self.results['hessian_tdv_result'][-1]
                            else:
                                frame_data = self.results['hessian_tdv_result']
                            stack_frames.append(frame_data)
                        
                        # 提取Hybrid+Background帧 (Hybrid result + Background)
                        if 'hybrid_result' in self.results and 'hybrid_background' in self.results:
                            # 提取Hybrid帧
                            if len(self.results['hybrid_result'].shape) == 3:
                                if internal_frame_idx < self.results['hybrid_result'].shape[0]:
                                    hybrid_frame = self.results['hybrid_result'][internal_frame_idx]
                            ...
                            # 将Hybrid和Background相加
                            hybrid_with_background = hybrid_frame + background_frame
                            stack_frames.append(hybrid_with_background)
                        
                        stack_name = f"{self.args.fname}_Frame{user_frame_idx}_Comparison_Type2"
            ...
            if success_count > 0:
                if export_options['frame_stack']:
                    stack_type_desc = "Type 1" if "Type 1" in export_options['stack_type'] else "Type 2"
                    self.status_label.setText(f"Exported {success_count} file(s) including {stack_type_desc} comparison to {save_dir}")
                else:
    ...
    def run_reconstruction(self):
        ...
        try:
            ...
            # 设置重建已开始标志
            self._reconstruction_started = True
    ...
    def handle_reconstruction_result(self, results):
        ...
        if 'hessian_result' in results:
            self.hessian_display.set_image(results['hessian_result'], current_frame, normalize=True)
            if len(results['hessian_result'].shape) == 3:
                max_frames = max(max_frames, results['hessian_result'].shape[0])
        
        if 'hessian_tdv_result' in results:
            self.hessian_tdv_display.set_image(results['hessian_tdv_result'], current_frame, normalize=True)
            if len(results['hessian_tdv_result'].shape) == 3:
                max_frames = max(max_frames, results['hessian_tdv_result'].shape[0])
        
        if 'hybrid_background' in results:
            self.hybrid_background_display.set_image(results['hybrid_background'], current_frame, normalize=True)
            if len(results['hybrid_background'].shape) == 3:
                max_frames = max(max_frames, results['hybrid_background'].shape[0])
