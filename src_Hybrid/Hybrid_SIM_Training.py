...
# 导入训练相关模块
from src.default_config import default_config_TDV_SIM_training
from src_BF.BF_SIM_Dataset import BF_SIM_Dataset
from src_TDV.TDV_DNN_Train import TDV_SIM_DNN_Train
...
class TrainingVisualizationWidget(QWidget):
    ...
    #     # self._log("Loss curves reset due to model reinitialization", is_debug=True)
    ...
    def init_ui(self):
        ...
        # 1. 标题
        title_label = QLabel("Hybrid-SIM Training Visualization")
    ...
    def add_training_loss_(self, batch_idx, loss):
        """添加训练损失数据点"""
    ...
    def add_valid_loss_(self, epoch, loss):
    ...
    def add_log(self, message):
        ...
        # # 检查是否需要重置损失曲线
        # if "reset_epoch_counter" in message or "Model reinitialized" in message:
        #     self.reset_loss_curves()
...
class TrainingThread(QThread):
    ...
    # reset_loss_curves_signal = pyqtSignal()  # 新添加：重置损失曲线信号
    best_loss_curves_signal = pyqtSignal(list, list, list, list)
    ...
    def run(self):
        try:
            ...
            if self.operation_type in ["dataset", "both"]:
                ...
                self.bf_sim = BF_SIM_Dataset(self.args, update_dataset_progress)
                
                if self._stop_requested:
                    self.bf_sim.stop_generation()
                    self.log_signal.emit("Dataset generation stopped before execution")
                    self.finished_signal.emit(False, "Stop training by the user")
                    return
                    
                Dataset_Clean, Dataset_Noisy = self.bf_sim.generation()
                ...
                if self.bf_sim and hasattr(self.bf_sim, 'is_stopped') and self.bf_sim.is_stopped():
            ...
            if self.operation_type in ["train", "both"]:
                ...
                #     """重置回调函数"""
                #     self.model_reset_signal.emit()
                    # self.log_signal.emit("Model reinitialized, resetting visualization...")
                ...
                # def reset_loss_curves_callback():
                #     """重置损失曲线回调函数"""
                #     self.reset_loss_curves_signal.emit()
                #     # self.log_signal.emit("reset_epoch_counter: Loss curves will be reset")
                
                self.tdv_dnn = TDV_SIM_DNN_Train(self.args, update_training_progress, log_callback)
            ...
            if not self._stop_requested:
                # pass
                self.finished_signal.emit(True, "Finish Hybrid-SIM training !")
    ...
    def cleanup_memory(self):
        ...
        try:
            ...
            # 2. 清理数据集对象
            if self.bf_sim is not None:
                try:
                    if hasattr(self.bf_sim, 'cleanup'):
                        self.bf_sim.cleanup()
                    del self.bf_sim
                    self.bf_sim = None
                except Exception as e:
                    self.log_signal.emit(f"Warning: Failed to clean BF-SIM: {str(e)}")
    ...
    def stop(self):
        ...
        if self.tdv_dnn:
            try:
                self.bf_sim = None
                self.tdv_dnn.stop_training()
                self.log_signal.emit("Stop TDV-DNN training ...")
            except:
                pass
...
class ParameterWidget(QWidget):
    ...
    def browse_file(self):
        ...
        if self.param_name == 'SIM_BF_PSF_path':
            ...
            file_filter = "TIFF Files (*.tif *.tiff);;All Files (*)"
        # elif self.param_name == 'SIM_raw_data_folder_path':
        ...
        if self.param_name == 'SIM_raw_data_folder_path':
...
class TDVTrainingGUI(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.args = default_config_TDV_SIM_training()
    ...
    def init_ui(self):        
        ...
        self.setWindowTitle("Hybrid-SIM Training")
        ...
        title_label = QLabel("Hybrid-SIM Training Parameters")
        title_label.setFont(QFont("Arial", 14, QFont.Bold))
        ...
        # BF-SIM 参数模块
        bf_sim_params = {
            'SIM_raw_data_folder_path': {
                'type': 'file',
                'default': './Demo_training_SIM',
                'tooltip': 'Path to SIM raw data folder'
            },
            'SIM_excitation_NA': {
                'type': 'float',
                'default': 1.4,
    ...
    def load_default_parameters(self):
        """加载默认参数"""
        for widget in [self.bf_sim_widget, self.dataset_widget, self.train_widget]:
    ...
    def load_previous_parameters(self):
        ...
        try:
            # 构建默认文件名
            try:
                filedir, filename = os.path.split(self.args.SIM_raw_data_folder_path)
                base_name = filedir +'/'
    ...
    def export_current_parameters(self):
        ...
        try:
            ...
            try:
                ...
                # 如果 fname 不存在，使用空字符串作为基础名称
                base_name = "Demo_training_SIM_Training_Parameters.json"
    ...
    def start_training(self):
        ...
        try:
            ...
            self.visualization_widget.add_log("Start Hybrid-SIM training ...")
            operation_text = self.operation_combo.currentText()
            ...
            # self.training_thread.reset_loss_curves_signal.connect(self.reset_loss_curves)  # 新添加
            self.training_thread.best_loss_curves_signal.connect(self.visualization_widget.set_best_loss_curves)
            # 连接重置信号
    ...
    # def reset_loss_curves(self):
    #     """重置损失曲线"""
    #     self.visualization_widget.reset_loss_curves()
    ...
    def stop_training(self):
        ...
        if self.training_thread and self.training_thread.isRunning():
            ...
            if reply == QMessageBox.Yes:
                ...
                # # 重要：先获取当前模型按钮状态，以便重置后恢复
                # model_btn_enabled = self.visualization_widget.model_path_btn.isEnabled()
                ...
                # # 恢复模型按钮状态
                # self.visualization_widget.model_path_btn.setEnabled(model_btn_enabled)
